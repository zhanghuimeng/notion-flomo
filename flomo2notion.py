...
class Flomo2Notion:
    ...
    def _count_file_blocks(self, page_id):
        """统计页面中的文件类型 block 数量"""
        blocks = self.notion_helper.get_block_children(page_id)
        return {
            'image': sum(1 for b in blocks if b['type'] == 'image'),
            'audio': sum(1 for b in blocks if b['type'] == 'audio'),
            'file': sum(1 for b in blocks if b['type'] == 'file'),
        }
    ...
    def sync_to_notion(self):
        ...
        self._mark_deleted_memos(all_flomo_slugs, deleted_slugs)
        print()
